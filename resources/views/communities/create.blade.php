<x-app-layout>
    <div class="py-8">
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="mb-6">
                <h1 class="text-2xl font-bold text-white">{{ __('Создать сообщество') }}</h1>
                <p class="text-surface-400 text-sm mt-1">{{ __('Объедините людей вокруг общей темы') }}</p>
            </div>

            <div class="card p-6 sm:p-8">
                <form action="{{ route('communities.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    @csrf

                    <div>
                        <x-input-label for="name" :value="__('Название')" />
                        <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="old('name')" required placeholder="{{ __('Название сообщества...') }}" />
                        <x-input-error class="mt-2" :messages="$errors->get('name')" />
                    </div>

                    <div>
                        <x-input-label for="description" :value="__('Описание')" />
                        <textarea id="description" name="description" rows="4" class="input-field mt-1" placeholder="{{ __('Расскажите о вашем сообществе...') }}">{{ old('description') }}</textarea>
                    ...
                    <div>
                        <label class="inline-flex items-center gap-2.5 px-4 py-3 rounded-xl bg-surface-700/30 border border-surface-600/30 cursor-pointer hover:border-brand-500/30 has-[:checked]:bg-brand-500/10 has-[:checked]:border-brand-500/30 transition-all duration-200">
                            <input type="checkbox" name="is_private" value="1" {{ old('is_private') ? 'checked' : '' }} class="rounded border-surface-600 bg-surface-800 text-brand-500 shadow-sm focus:ring-brand-500/50 focus:ring-offset-0">
                            <div>
                    ...
                    <div class="flex items-center gap-4 pt-4 border-t border-surface-700/30">
                        ...
                        <a href="{{ route('communities.index') }}" class="text-sm text-surface-400 hover:text-surface-200 transition-colors">{{ __('Отмена') }}</a>
                    </div>
                </form>
            </div>
        </div>
